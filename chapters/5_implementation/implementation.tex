...
\todo{Überschrift gefolgt von Überschrift}
...
Hence the current implementation uses Calpa API as default case, \todo{Ich würde case streichen. Dann neuer Satz: In the config file there is...} there is only one boolean property called \texttt{useFlyingSaucer} that indicates to use the \texttt{DescriptionPaneFS} class, a description pane that uses the  FlyingSaucer API.
...
Furthermore, the name of the \enquote{\texttt{useFlyingSaucer}} \todo{hier gibt es quotes, oben nicht.} property does not indicate very well what functionality is exactly configured.
...
For the JavaFX WebView based description pane, a new class \texttt{DescriptionPaneFX} is generated, that uses a \texttt{JFXPanel} that contains a Java FX \todo{im Dokument überprüfen dass JavaFX immer gleich geschreiben wird} WebView for visualising HTML content.
...
Figure \ref{fig:class_diag_desc_pane_fx} demonstrates the class structure of the FX based description pane. \todo{Da fragt sich der geneigte Leser: Dauert das laden der JavaFX runtime environment nicht ewig und das gesamte Programm freezt?}
...
Although this library is shipped with the JRE since Java SE 7 update 6 (7u6), it is not part of the default classpath that is used when building and starting Netbeans projects. Thus, it is not possible to use and built programs that use JavaFX dependent classes within the Netbeans IDE. \todo{Wo kommt jetzt Netbeans und Maven her? Wurde evt. bereits erwähnt. Wäre aber ein guter Satz für oben zwischen die Überschriften.}
...
Therefore an other \todo{another} solution is preferred.
The jfxrt.jar \todo{Was ist das?} gets defined as system dependency.
System dependencies are always available and are not looked up in the maven specific repository, hence maven assumes that these dependencies are explicitly provided. \todo{Im Dokument überprüfen dass Maven immer gross geschrieben wird.}
Thus way, the JavaFX dependency can be defined in a system and JRE independent way. \todo{Thus way? This way ist gemeint?}
Additionally it is necessary to add the library to the classpath that is created from Maven for running the application and to avoid a \texttt{java.lang.NoClassDefFoundError} when running the application. \todo{Mir ist nicht ganz klar wie das jetzt funktioniert. Evt. sind die Details aber nicht so wichtig. Muss die System dependency für jeden Benutzer einzeln eingestellt werden? Die Einstellung für Maven bezieht sich auf diese System dependency, oder wird JavaFX dort "normal" hinzugefügt?}
...
To fully understand what possibilities exists \todo{exist} to implement the pending feature, it is necessary to have a closer look on that mechanism.
...
If the selected node is an object node, it is necessary to check if for the corresponding cids class the new introduced class attribute \texttt{isHtmlRenderer} is configured. \todo{...if for... finde ich komisch}
...
The drawback of this approach is, that it will only work when \todo{ist when hier richtig? if evt?} the new JavaFX based description pane is used and will not work with the other description pane implementations.
...
Hence the only difference for web renderer components is the URL that  points to the location of the web renderer which is configured in the isHtmlRenderer \todo{texttt} class attribute, it is easily possible to develop a new renderer class \texttt{HTMLWidgetRenderer} that uses also a \texttt{FXBrowserPanel}, and is parametrisable  with the URL of the web application.

In fact this component can also be used for visualising web based editors, \todo{Punkt} class inheritance can be used to inherit the needed functionality from the editor to the renderer and allows the same code base for both cases.
However, this approach heavily relies on the assumption that renderer and editors for single objects have only a different intentional usage and no different graphical user interfaces. \todo{Einzahl?}
A renderer visualizes \todo{eher AE schreibweise, geht auch in BE aber sonst immer s benutzt} information whether a editor can be used to edit the data.
...
Important to note, is the \texttt{editable} parameter in the constructor of the \texttt{HTMLWidgetEditor}. \todo{Hier gibt es bei mir eine Overfull \\hbox}
...
Without such an application it would be difficult, if not impossible, to test \todo{the} above mentioned enhancements.
...
Area \todo{Wurde ich auch wieder Part nennen. Evt. auch irgendwie Part x hervorheben} c in figure \ref{fig:survey_plan_swing_editor} shows that the survey plan renderer can also visualise images of historic survey plans documents.
...
\todo{Bei mir ist dieses Listing auf zwei seiten verteilt und das Bild ist dazwischen}
...
The look of the application can be easily adopted with some additional style sheets to gain a similar design to the already existing swing \todo{im gesamten Dokument drauf achten dass Swing gleich geschrieben wird} components. This supports a seamless integration into the cids navigator.
...
The ng-hide \todo{texttt, sicherheitshalber mal im Dokument nach ng- suchen} directive hides arbitrary HTML elements based on the expression that is provided to it.
...
An example is the format field of the survey plan document. A survey plan has a format and there is a fix set of formats that exists. \todo{Ich weiß was du meinst.}
In the Swing application \texttt{ComboBoxes} are used to offer the user this fix set of elements of the related cids class.
The \texttt{CidsBean} that represents a survey plan only contains just the currently selected element of the related cids class however. \todo{Ich würde just und however streichen} 
...
To ease the integration of this feature afterwards, a Java Script \todo{Java Script immer gleich schreiben} function that loads all elements of a given cids object is implemented.
Hence no data exchange with the cids system is possible, those information are also loaded from a json \todo{JSON immer gleich schreiben} file based on a naming convention.
...
In fact the bridge object can be registered at an arbitrary DOM node or JavaScript object, \todo{Punkt oder übergang zwischen den Sätzen.} it is necessary to establish a common interface on which the bridge object always is registered to allow a unified usage within the web application context. \todo{Das always stört mich. Andere Stelle evt.}
...
Just if this happens, it is possible to forward this data to the web application. \todo{Zwei letzte Sätze sind vom Satzbau her komisch. In Fact und Just.}

Since it is necessary to regard \todo{Du meinst ja hier, dass der Renderer im Browser angeschaut werden kann? Wenn ja, dann ist regard (etwas ansehen als) das falsche Verb. M.E. ist nicht wirklich ersichtlich was gemeint ist.} the usage of the web renderer component also without the navigator this  behaviour needs to be abstracted from the application itself.
...
Placing the initialisation function in a separate file it is sufficient to include this file with a script tag in the index file of the application to use the cidsJS layer object. \todo{schwer verständlich}
...
Hence there is no guaranteed order that the \texttt{SurveyPlanController} is initialised after the \texttt{CidsBean} was injected into cidsJS, the initialisation of the controller needs to be delayed. \todo{Warum order? There is not guarantee that X happens before Y. Warum muss die initialisation delayed werden, wenn eh ein Promise benutzt wird? }
Therefore the \texttt{getCidsBean} method returns a promise object.
Promises are a very common concept in JavaScript to handle asynchronous events. \todo{Ist Promise nicht ein Klassenname?}
...
In our case this defer object is initialized during the initialisation of cidsJS and resolved when the injectCidsBean \todo{textfff} method is called. The sequence diagramm \ref{fig:seq-diag-data-exchange-1} demonstrates the steps that are executed during the initialisation process. \todo{Evt. Bild eher bringen. Der Leser, d.h. ich, baut sich das ansonsten selbst im Kopf zusammen und guckt dann nur mal drüber ob er alles richtig verstanden hat. Umgedreht wäre wohl einfacher.}
...
However, using the demo application as editor is still not possible in fact \todo{because, in fact finde ich hier nicht passend} only a one way data exchange is implemented so far. 
...
The watch expression represents the value that will be watched which in our case this \todo{this zuviel} is the value of each property of the cidsBean. 
\todo{D.h. eine Watch für jede Property? Satzbau, evt zwei Sätze?}
...
Hence the main reason of the watches isnjust \todo{is just?} to indicate that the data has changed, the watches are only needed until the first change in the data has happened. 
...
To solve the above mentioned problem, the de-register functions are saved in an array when the watches are set up. The callback function of the watches uses the bridge object to mark the \texttt{CidsBean} as changed. Afterwards, all de-register functions to disable watch-expressions on \todo{on zuviel} are executed. 
...
Because it is not clear what properties of the \texttt{CidsBean} have changed, the \texttt{CidsBean} class is extended with a method \texttt{bulkUpdate(CidsBean otherBean)} that updates the properties of a \texttt{CidsBean} according to an other \todo{another} instance of a \texttt{CidsBean}. 
...
In fact Angular does not fire a event for that case, it is necessary to understand how an angular application is bootstrapped. \todo{Angular wird hier häufiger mal gross mal klein geschrieben.}
...
It is initialized with a given value. Additionally the await \todo{texttt} method of the \texttt{CountDownLatch} will block until the value of it reaches zero. 
The thread in that the \texttt{DescriptionPane} loads the renderer needs to check if the renderer \todo{currently loaded renderer} that is currently loaded implements the \texttt{InitialisationLocker} interface and calls the await method of the \texttt{CountDownLatch} this interface provides. \todo{... and calls its \\texttt\{await\} method}
This causes to block the thread of the loading mechanism. 
If the web application sends the message that it is initialised, the \texttt{HtmlWidgetEditor} counts down the value of the \texttt{CountDownLatch} which reverses the blocked status renderer loading thread. \todo{CountDownLatch evt. mit Semaphoren vergleichen. M.E. nicht klar ob CountDownLatch selbst erfunden oder aus Bibliothek.}
