...
Withal \todo{Laut Leo ist withal veraltet. Mir war des Wort nicht bekannt.}, the degree how seamless a web application is integrated, is very scalable and reaches from the simple display of web applications to highly integrated components, that interact with the already existing Swing based navigator GUI and behave like already existing Swing renderer. \todo{plural} 
...
Using the already existing class attributes it is possible to gain \todo{benefit, kA ob gain passt} from the already existing support in the cids management tools. 
...
Hence the JavaFX is much more feature rich than the two browser API's that can be used up to now, totally \todo{ist totally hier das passende Wort} new possibilities in the design of description pages are available now. 
...
Another important issue that was not regarded in the implementation is to extend the survey plan application, to use different styles, depending on the context it is nested in. \todo{it is nested in gefällt mir nicht} 
...
Regarding the bi-directional data exchange of the navigator and integrated web applications, it is clear that there is also open work to do. \todo{den Ausdruck open work gibt es so glaube ich nicht. Evt. some issues are still open} 
